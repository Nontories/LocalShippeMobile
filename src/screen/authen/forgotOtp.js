...
import { forgotOtp } from "../../api/auth"
...
const ForgotOtp = ({ route, navigation }) => {
    ...
    const checkOtp = async () => {
        ...
        setLoading(true)
        const response = await forgotOtp(mail, otp);
        console.log(response.data);
        if (response?.status === 200) {
            getData(response.data)
            setLoading(false)
            navigation.navigate("NewPassword" , { mail: mail })
        } else {
            setLoading(false)
        }
    }
